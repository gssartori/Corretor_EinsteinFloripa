...
from relatorios.functions.utils import edita_status
...
def cria_simulados():
    ...
    # template = env.get_template(default_storage.open("simulinho_aluno_template.html"))
    ...
    for aluno in df_q.index.get_level_values(0).unique():
        ######## MUDANDO O STATUS DE PENDENTE PARA GERANDO ##########
        ...
        for materia in df_q_aluno.index.get_level_values(0).unique():
            ...
            df_hide_index = df_q_aluno_loc.hide_index().render()
            doc['materias'].append({'materia': materia, 'df': df_hide_index})
        ...
        constroi_email(html_out, doc['nome'], 'gabriel.s@autojun.com.br')
